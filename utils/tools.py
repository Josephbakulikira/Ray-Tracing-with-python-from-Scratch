...
def intersect_world(world, ray):
    ...
    for ob in world.Objects:
        xs = intersect(ray, ob)
        [_intersections.append(element) for element in xs]
...
def shade_hit(world, comps, remaining):
    shadw = is_shadowed(world, comps.over_point)
    surface = Lighting(comps.shape.material, comps.shape, world.light, comps.over_point, comps.eye_vector, comps.normal_vector, shadw)
